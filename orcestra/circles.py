...
import pandas as pd
ds = xr.open_dataset("ipfs://bafybeihfqxfckruepjhrkafaz6xg5a4sepx6ahhv4zds4b3hnfiyj35c5i", engine="zarr")
...
plt.legend()
# %%
ex = ds.swap_dims({"circle":"circle_time"}).sel(circle_time="2024-09-23-12:00",method="nearest")
for circle_time in ex.circle_time.values:
    print(circle_time)
    dt = pd.Timestamp(circle_time).to_pydatetime()
    ex.sel(circle_time=circle_time).omega.plot(y="altitude", label=f"{dt:%m-%d %H:%M}")
...
ex = ds.swap_dims({"circle":"circle_time"}).sel(circle_time="2024-09-23-10:00",method="nearest")
dt = pd.Timestamp(ex.circle_time.values).to_pydatetime()
ex.omega.plot(y="altitude", ax=ax, label=f"{dt:%m-%d %H}hr", lw=2, c=colors["West"])

ex = ds.swap_dims({"circle":"circle_time"}).sel(circle_time="2024-09-03-15:00",method="nearest")
dt = pd.Timestamp(ex.circle_time.values).to_pydatetime()
...
sns.set_context("paper", font_scale=1.)
ex = ds.swap_dims({"circle":"circle_time"}).sel(circle_time="2024-09-03-18:30",method="nearest")
dt = pd.Timestamp(ex.circle_time.values).to_pydatetime()
